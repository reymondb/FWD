...
SUM(CASE WHEN vicidial_list.called_count >=6 THEN 1 ELSE 0 END) AS total6
    ...
    public function fetchLeadStats(Request $request)
    {
        ...
        $data = DB::connection('mysql_external')
            ...
            DB::raw('(SELECT COUNT(DISTINCT vicidial_list.phone_number) FROM `vicidial_list` WHERE vicidial_list.list_id="'.$request->list_id.'") as total_leads'),
            DB::raw('(SELECT COUNT(vicidial_list.list_id) FROM `vicidial_list` left join vicidial_log on vicidial_list.lead_id=vicidial_log.lead_id  and vicidial_log.list_id = "'.$request->list_id.'"  WHERE vicidial_list.list_id="'.$request->list_id.'") as overalltotal'),
            ...
            ->leftjoin('vicidial_log', function($join) use ($list_id)
            {
                ...
                $join->where('vicidial_log.list_id', '=', $list_id);
            })
            ->leftjoin('vicidial_campaign_statuses', function($join)
            {
                $join->on('vicidial_campaign_statuses.status', '=', 'vicidial_list.status');
                $join->where(DB::raw('LOWER(vicidial_campaign_statuses.campaign_id)'), '=', DB::raw('LOWER(vicidial_log.campaign_id)'));
            }) 
